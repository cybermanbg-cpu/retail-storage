...
class ReportController extends Controller
{
    ...
    public function productSales(Request $request)
    {
        ...
        foreach ($products as $product) {
            $sales = ReceiptItem::whereHas('receipt', function ($q) use ($startDate, $endDate, $ownerId) {
                $q->whereBetween('created_at', [$startDate, $endDate])->where('type', 'sale');
                if ($ownerId) {
            ...
            $purchases = PurchaseItem::whereHas('purchase', function ($q) use ($startDate, $endDate, $ownerId) {
                ...
                ->select(DB::raw('SUM(quantity) as total_purchased'), DB::raw('SUM(total_cost) as total_cost'))
                ->first();
            ...
            $report[] = [
                ...
                'margin' => $sales->total_revenue > 0 ? (($sales->total_revenue - ($purchases->total_cost ?? 0)) / $sales->total_revenue) * 100 : 0,
            ];
        }
    ...
    public function clientSales(Request $request)
    {
        ...
        foreach ($clients as $client) {
            $sales = Receipt::where('client_id', $client->id)
                ...
                ->select(DB::raw('COUNT(*) as receipt_count'), DB::raw('SUM(total_amount) as total_revenue'), DB::raw('SUM(total_vat) as total_vat'))
                ->first();
            ...
            $report[] = [
                ...
                'avg_receipt' => ($sales->receipt_count ?? 0) > 0 ? $sales->total_revenue / $sales->receipt_count : 0,
            ];
        }
    ...
    public function cashierSales(Request $request)
    {
        ...
        foreach ($cashiers as $cashier) {
            $sales = Receipt::where('user_id', $cashier->id)
                ...
                ->select(DB::raw('COUNT(*) as receipt_count'), DB::raw('SUM(total_amount) as total_revenue'), DB::raw('SUM(total_vat) as total_vat'), DB::raw('COUNT(DISTINCT client_id) as unique_clients'))
                ->first();
    ...
    public function monthlySales(Request $request)
    {
        ...
        $sales = Receipt::whereYear('created_at', $year)
            ...
            ->select(DB::raw('MONTH(created_at) as month'), DB::raw('COUNT(*) as receipt_count'), DB::raw('SUM(total_amount) as total_revenue'), DB::raw('SUM(total_vat) as total_vat'))
            ->groupBy('month')
            ->orderBy('month')
            ->get();
        ...
        $purchases = Purchase::whereYear('purchase_date', $year)
            ...
            ->select(DB::raw('MONTH(purchase_date) as month'), DB::raw('SUM(total) as total_cost'))
            ->groupBy('month')
            ->orderBy('month')
            ->get();
    ...
    public function profitAnalysis(Request $request)
    {
        ...
        foreach ($salesItems as $item) {
            // Проверка дали productVariant съществува и има продукт
            if (!$item->productVariant) {
                // Ако няма вариант, използваме snapshot данните
                ...
                $report[] = [
                    ...
                    'variant' => 'Няма вариант',
                    'quantity' => $quantity,
                    'unit' => $item->unit_of_measure_snapshot ?? 'бр.',
            ...
            if (!$item->productVariant->product) {
                // Ако няма продукт, пропускаме или използваме snapshot
                continue;
            }
        ...
        $productSummary = collect($report)
            ...
            ->map(function ($items, $productName) {
                return [
                    ...
                    'margin_percent' => $items->sum('revenue') > 0 ? ($items->sum('profit') / $items->sum('revenue')) * 100 : 0,
                ];
            })
            ->sortByDesc('profit')
            ->values();
    ...
    private function getCostPerUnit($productId, $storageObjectId, $date)
    {
        ...
        if (!$productId || !$storageObjectId) {
            return 0;
        }

        $result = DB::table('purchase_items')->select('purchase_items.final_unit_cost')->join('purchases', 'purchase_items.purchase_id', '=', 'purchases.id')->join('product_variants', 'purchase_items.product_variant_id', '=', 'product_variants.id')->join('products', 'product_variants.product_id', '=', 'products.id')->where('products.id', $productId)->where('purchases.storage_object_id', $storageObjectId)->where('purchases.purchase_date', '<=', $date)->orderBy('purchases.purchase_date', 'desc')->first();

        if ($result && isset($result->final_unit_cost)) {
            return (float) $result->final_unit_cost;
        }
        ...
        return $stock && $stock->average_cost ? (float) $stock->average_cost : 0;
    }
    ...
    public function productPurchases(Request $request)
    {
        ...
        foreach ($products as $product) {
            $purchases = PurchaseItem::whereHas('purchase', function ($q) use ($startDate, $endDate, $ownerId) {
                ...
                ->select(DB::raw('SUM(quantity) as total_quantity'), DB::raw('SUM(total_cost) as total_cost'), DB::raw('AVG(final_unit_cost) as avg_cost'))
                ->first();
    ...
    public function stockStatus(Request $request)
    {
        ...
        $query = Stock::with(['productVariant.product', 'storageObject'])->whereHas('productVariant.product', fn($q) => $q->where('type', 'product'));
    ...
    public function hourlySales(Request $request)
    {
        ...
        $sales = Receipt::whereDate('created_at', $date)->where('type', 'sale')->select(DB::raw('HOUR(created_at) as hour'), DB::raw('COUNT(*) as receipts'), DB::raw('SUM(total_amount) as total'))->groupBy('hour')->orderBy('hour')->get();
    ...
    public function topProducts(Request $request)
    {
        ...
        $topProducts = ReceiptItem::select('product_name_snapshot', DB::raw('SUM(quantity) as total_quantity'), DB::raw('SUM(total) as total_revenue'))->whereHas('receipt', fn($q) => $q->where('created_at', '>=', $startDate))->groupBy('product_name_snapshot')->orderByDesc('total_revenue')->limit(10)->get();
    ...
    {
        $customers = Client::withSum('receipts as total_spent', 'total_amount')->withCount('receipts')->where('is_active', true)->orderByDesc('total_spent')->get();
    ...
    public function dailyTurnover(Request $request)
    {
        ...
        $daily = Receipt::whereYear('created_at', $year)->whereMonth('created_at', $month)->where('type', 'sale')->select(DB::raw('DAY(created_at) as day'), DB::raw('SUM(total_amount) as total'))->groupBy('day')->get()->keyBy('day');
    ...
    public function monthlyProfit(Request $request)
    {
        ...
        $revenue = Receipt::whereYear('created_at', $year)->where('type', 'sale')->select(DB::raw('MONTH(created_at) as month'), DB::raw('SUM(total_amount) as total'))->groupBy('month')->get()->keyBy('month');

        $costs = Purchase::whereYear('purchase_date', $year)->select(DB::raw('MONTH(purchase_date) as month'), DB::raw('SUM(total) as total'))->groupBy('month')->get()->keyBy('month');
        ...
        foreach ($months as $i => $name) {
            $monthNum = $i + 1;
            $inc = isset($revenue[$monthNum]) ? $revenue[$monthNum]->total : 0;
            $out = isset($costs[$monthNum]) ? $costs[$monthNum]->total : 0;
            $report[] = [
                'month' => $name,
                'revenue' => $inc,
                'costs' => $out,
    ...
    public function dashboard()
    {
        ...
        // Общи статистики
        $totalProducts = Product::where('type', 'product')->when($ownerId, fn($q) => $q->where('owner_id', $ownerId))->count();

        $totalClients = Client::when($ownerId, fn($q) => $q->where('owner_id', $ownerId))->count();

        $totalCashiers = User::role('cashier')->when($ownerId, fn($q) => $q->where('owner_id', $ownerId))->count();

        // Продажби днес
        $todaySales = Receipt::where('type', 'sale')->whereDate('created_at', today())->when($ownerId, fn($q) => $q->where('owner_id', $ownerId))->sum('total_amount');

        $todayReceipts = Receipt::where('type', 'sale')->whereDate('created_at', today())->when($ownerId, fn($q) => $q->where('owner_id', $ownerId))->count();

        // Продажби този месец
        $monthSales = Receipt::where('type', 'sale')->whereYear('created_at', now()->year)->whereMonth('created_at', now()->month)->when($ownerId, fn($q) => $q->where('owner_id', $ownerId))->sum('total_amount');
        ...
        $topProduct = ReceiptItem::select('product_name_snapshot', DB::raw('SUM(quantity) as total_qty'))
            ->whereHas('receipt', function ($q) use ($ownerId) {
                $q->where('type', 'sale')->whereYear('created_at', now()->year)->whereMonth('created_at', now()->month);
                if ($ownerId) {
                    $q->where('owner_id', $ownerId);
                }
            })
        ...
        $topClient = Receipt::select('client_id', DB::raw('SUM(total_amount) as total'))->where('type', 'sale')->whereYear('created_at', now()->year)->whereMonth('created_at', now()->month)->whereNotNull('client_id')->when($ownerId, fn($q) => $q->where('owner_id', $ownerId))->groupBy('client_id')->orderByDesc('total')->with('client')->first();

        return view('reports.dashboard', compact('totalProducts', 'totalClients', 'totalCashiers', 'todaySales', 'todayReceipts', 'monthSales', 'topProduct', 'topClient'));
    }
}
