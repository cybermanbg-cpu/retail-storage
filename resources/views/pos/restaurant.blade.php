...
<div class="container-fluid px-4 py-3 h-screen flex flex-col">
    ...
    <!-- Top Bar -->
    <div class="flex justify-between items-center mb-4 bg-white p-4 rounded-2xl shadow">
        <div class="flex items-center gap-4">
            <h1 class="text-3xl font-bold text-gray-800 flex items-center gap-3">
                <i class="fas fa-utensils text-green-600"></i>
                Restaurant POS
            </h1>
            <div class="bg-green-100 text-green-700 px-5 py-2 rounded-xl font-semibold text-lg">
                Маса № <span id="tableNumber" class="text-2xl">12</span>
                <button onclick="changeTable()" class="ml-2 text-sm bg-green-200 px-2 py-1 rounded-lg">
        ...
        <div class="flex items-center gap-6">
            <div class="text-right">
                <div class="text-sm text-gray-500">Обект:</div>
                <div class="font-semibold">{{ $storageObject->name }}</div>
            </div>
            <div class="text-right">
                <div class="text-sm text-gray-500">Касиер:</div>
    ...
    <div class="flex-1 grid grid-cols-12 gap-4 min-h-0">
        ...
        <!-- Категории -->
        <div class="col-span-2 bg-white rounded-2xl shadow p-4 overflow-y-auto">
            ...
            <div class="grid grid-cols-2 gap-3" id="categoriesGrid">
                ...
                <button class="category-btn aspect-square flex flex-col items-center justify-center p-4 rounded-2xl transition-all {{ $loop->first ? 'bg-primary-600 text-white shadow-md' : 'bg-gray-100 hover:bg-gray-200' }}"
                    ...
                    @if($category->default_discount > 0)
                        <span class="text-xs mt-1 bg-red-500 text-white px-2 py-0.5 rounded-full">-{{ $category->default_discount }}%</span>
                    @endif
                </button>
                @endforeach
            </div>
        ...
        <!-- Продукти -->
        <div class="col-span-7 bg-white rounded-2xl shadow p-4 flex flex-col">
            <div class="mb-4 flex gap-3">
                <input type="text" id="searchInput" 
                       class="flex-1 px-5 py-4 text-lg border-2 border-gray-200 rounded-2xl focus:border-primary-500 outline-none"
                       placeholder="🔍 Търси продукт по име...">
                <button id="clearSearchBtn" class="px-5 py-4 bg-gray-200 rounded-2xl hover:bg-gray-300">
                    <i class="fas fa-times"></i>
                </button>
            </div>
            
            <div id="productsGrid" class="grid grid-cols-3 md:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-4 flex-1 overflow-y-auto">
                <!-- Products loaded by JS -->
            </div>
        </div>

        <!-- Количка -->
        <div class="col-span-3 bg-white rounded-2xl shadow flex flex-col">
            ...
            <div class="p-5 border-t bg-gray-50 rounded-b-2xl">
                ...
                <div class="flex justify-between text-lg text-red-600 mb-2">
                    <span>Отстъпка:</span>
                    <span id="discountAmount">0.00 €</span>
                </div>
                <div class="flex justify-between text-2xl font-bold mb-4 pt-2 border-t">
                    <span>Общо:</span>
                ...
                <div class="grid grid-cols-2 gap-3">
                    <button id="clearCartBtn" 
                            class="bg-gray-300 hover:bg-gray-400 text-gray-800 text-lg font-bold py-3 rounded-xl transition">
                          <i class="fas fa-trash-alt mr-2"></i>
                        Изчисти
                    </button>
                    <button id="checkoutBtn" 
                            class="bg-green-600 hover:bg-green-700 text-white text-lg font-bold py-3 rounded-xl transition shadow-lg">
...
<div id="paymentModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 hidden overflow-y-auto h-full w-full z-50">
    <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-2xl bg-white">
        ...
        <div class="space-y-3">
            <button onclick="processPayment('card')" class="w-full bg-blue-600 hover:bg-blue-700 text-white py-3 rounded-xl">
                <i class="fas fa-credit-card mr-2"></i> Карта
            </button>
            <button onclick="processPayment('cash')" class="w-full bg-green-600 hover:bg-green-700 text-white py-3 rounded-xl">
                <i class="fas fa-money-bill mr-2"></i> В брой
            </button>
            <button onclick="closePaymentModal()" class="w-full bg-gray-300 hover:bg-gray-400 text-gray-800 py-3 rounded-xl">
                Отказ
            </button>
        </div>
    </div>
...
function processPayment(method) {
    ...
    $.ajax({
        ...
        success: function(res) {
            if (res.success) {
                alert(`✅ Поръчка №${res.receipt_number} на маса ${tableNumber} е записана успешно!\nПлащане: ${method === 'card' ? 'Карта' : 'В брой'}`);
                currentItems = [];
                updateCart();
                closePaymentModal();
            } else {
